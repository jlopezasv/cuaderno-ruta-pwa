...
import { cancelOperationalSession } from "../aggregate/OperationalSession.js";

/**
 * Cancela sesión operativa abierta.
 */
export class CancelarOperationalSessionCommand {
  /**
   ...
   * @param {string} [reason]
   */
  async execute(expeditionId, sessionId, reason) {
    ...
    try {
      const { session, events } = cancelOperationalSession(current, reason);
