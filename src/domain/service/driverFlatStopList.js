...
export async function fetchDriverOperationalCandidates(uid) {
  if (!uid) return { candidates: [], assignedAtById: {}, participacionBySvId: {} };
...
export async function resolveDriverFlatPendingStops(uid, { conductorNameById = {} } = {}) {
  ...
  const { candidates, assignedAtById, participacionBySvId } = await fetchDriverOperationalCandidates(uid);
