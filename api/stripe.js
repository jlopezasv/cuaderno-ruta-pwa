...
export default async function handler(req, res) {
  ...
  if (typeof body === 'string') {
    try { body = JSON.parse(body); } catch(_) { return res.status(400).json({ error: 'JSON inválido' }); }
  }
  ...
  if (action === 'create_checkout') {
    if (!user_id || !email || !plan) return res.status(400).json({ error: 'Faltan datos' });
  ...
  if (action === 'check_subscription') {
    ...
    try {
      ...
      if (!rows.length) {
        ...
        return res.status(200).json({ status: 'trial', trial_ends_at: trialEnd, days_left: 14 });
      }
      const sub = rows[0];
      const now = new Date();
      if (sub.status === 'trial') {
        ...
        if (daysLeft <= 0) return res.status(200).json({ status: 'expired', days_left: 0 });
        return res.status(200).json({ status: 'trial', trial_ends_at: sub.trial_ends_at, days_left: daysLeft });
      }
      if (sub.status === 'active') {
        return res.status(200).json({ status: 'active', plan: sub.plan, current_period_end: sub.current_period_end });
      }
